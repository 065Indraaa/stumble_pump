...
export function buildLobby() {
  ...
  // Main 3D Tile Platform (Cyan and Pink like Stumble Guys)
  const floorHeightFn = () => -1;
  const floor = make3DTileFloor(16, 16, 4, floorHeightFn, 0x4A90E2, 0xFF5CA8);
  group.add(floor);

  // Soft bumper edge (Stumble Guys style padding)
  const edgeGeo = new THREE.TorusGeometry(28, 1.2, 16, 48);
  const edgeMat = toonMat(0xFF5CA8); // bright pink bumper
  ...
  for (let i = 0; i < 8; i++) {
    ...
    const bump = new THREE.Mesh(new THREE.CylinderGeometry(2, 2, 0.5, 16), toonMat(0x5FCB88));
